...
class tool_manager {
    ...
    /**
     * Collects extra file extensions a tool wants the main upload field to accept, in addition to
     * the core ".pdf" (e.g. leafrtool_office offering ".docx"/".pptx" when a file converter is
     * configured on the site).
     *
     * @return string[] Extra accepted extensions, e.g. ['.docx', '.pptx']
     */
    public static function get_accepted_file_types(): array {
        $types = [];
        foreach (self::get_enabled_tools() as $component) {
            $tool = component_callback($component, 'get_accepted_file_types', [], []);
            if (is_array($tool)) {
                $types = array_merge($types, $tool);
            }
        }
    ...
    /**
     * Lets every enabled tool react to the main file having been (re)saved, e.g. to notice a
     * non-PDF upload and queue its background conversion to PDF.
     *
     * @param int $leafrid Leafr instance id
     * @param \context_module $context Module context
     */
    public static function handle_content_saved(int $leafrid, \context_module $context): void {
        foreach (self::get_enabled_tools() as $component) {
            component_callback($component, 'handle_content_saved', [$leafrid, $context]);
        }
    }

    /**
     * Asks every enabled tool whether it is responsible for turning the uploaded file into the PDF
     * the reader should display (e.g. leafrtool_office for an uploaded Word/PowerPoint file). Only
     * called when the uploaded file itself is not already a PDF.
     *
     * @param int $leafrid Leafr instance id
     * @param \context_module $context Module context
     * @return array|null ['status' => 'pending'|'ready'|'failed', 'file' => \stored_file|null], or
     *  null if no installed tool is responsible for this file.
     */
    public static function get_converted_file(int $leafrid, \context_module $context): ?array {
        foreach (self::get_enabled_tools() as $component) {
            $result = component_callback($component, 'get_converted_file', [$leafrid, $context]);
    ...
    /**
     * Lets an enabled tool replace the file offered for download (e.g. leafrtool_office offering
     * the original Word/PowerPoint file instead of the converted PDF, per its own setting).
     *
     * @param int $leafrid Leafr instance id
     * @param \context_module $context Module context
     * @param \stored_file $default The file that would be offered without a tool's involvement
     * @return \stored_file
     */
    public static function get_download_file(int $leafrid, \context_module $context, \stored_file $default): \stored_file {
        foreach (self::get_enabled_tools() as $component) {
            $override = component_callback($component, 'get_download_file', [$leafrid, $context, $default]);
            if ($override instanceof \stored_file) {
                return $override;
            }
        }
        return $default;
